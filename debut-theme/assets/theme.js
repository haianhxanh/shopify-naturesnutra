...
// const targetWidth = 749;
// if (w>targetWidth) {
  // const zoomOnClick = document.querySelectorAll('.product-single__media--has-thumbnails');
  const zoomOnClick = document.querySelectorAll('.feature-row__image.product-featured-media');

  zoomOnClick.forEach((image) => {
    let click = 0;
    image.addEventListener('click', () => {
      const thumbnails = document.querySelector('.product-single__thumbnails-product-template');
      const thumbnailsWrappper = document.querySelector('.thumbnails-wrapper');
      click++;
      image.classList.toggle('zoom-in');
      thumbnailsWrappper.classList.toggle('hide-thumbnails')
      thumbnails.classList.toggle('hide-thumbnails');
      if (click%2===1) {
